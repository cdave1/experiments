...
void loadShadowShader()
{
	...
	vertexShaderHandle   = loadShader("/Users/davidpetrie/Downloads/src/ddd/ddd/VertexShader.c",GL_VERTEX_SHADER);
	fragmentShaderHandle = loadShader("/Users/davidpetrie/Downloads/src/ddd/ddd/FragmentShader.c",GL_FRAGMENT_SHADER);
...
//	gluPerspective(45,RENDER_WIDTH/RENDER_HEIGHT,10,40000);
    {
        float w = 0.5 * .1 * RENDER_WIDTH;
        float h = 0.5 * 0.1 * RENDER_HEIGHT;
        glOrtho(-w, w, -h, h, 10, 40000);
    }
...
void update(void)
{
    ...
    p_camera[0] = light_mvnt * -cos(glutGet(GLUT_ELAPSED_TIME)/500.0);
    p_camera[2] = light_mvnt * -sin(glutGet(GLUT_ELAPSED_TIME)/500.0);
}
